...
describe("Subscription Metrics Calculation", () => {
  ...
  // Mirror of the route's aggregateCancellationReasons helper (kept local because
  // the route module cannot be imported in the test env - it builds a Supabase
  // client at import time).
  const aggregateCancellationReasons = (
  ...
  it("aggregates cancellation reasons by count (ignoring blank reasons)", () => {
    const rows = [
    ...
    expect(aggregateCancellationReasons(rows)).toEqual([
      { reason: "too_expensive", count: 2 },
      { reason: "not_using", count: 1 },
    ]);
  });
  ...
  it("resolves subscription price correctly from multiple sources", () => {
    ...
    expect(resolveSubscriptionPrice(sub0, mockTierMap, billingMap)).toBe(
      150000,
    );
...
describe("Admin Subscription Actions Validation", () => {
  ...
  it("validates reactivate action eligibility", () => {
    const canReactivate = (status: string) => {
      return ["cancelled", "grace_period", "expired", "paused"].includes(
        status,
      );
    };
